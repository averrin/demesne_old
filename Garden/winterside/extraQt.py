...
from winterside.snowflake import getFileContent, CWD
from winterside.base import WinterObject
#import regex as re
import sys
...
class WinterLine(QLineEdit):
    ...
    def setComplete(self, array):
        self.completerList = []
        for line in array:
            self.completerList.append(str(line))
    ...
    class CustomLexer(QsciLexerCustom):
        ...
        def styleGroup(self, pattern, style, off=0):
            ...
            for line in self.source.splitlines(True):
                ...
                #            pos=rx.indexIn(str(str(line)))
                matches = {}
                pos = 0
                last = ''
                if style:
                    while rx.indexIn(str(str(line)), pos) != -1:
                        token = rx.cap(1)
                        pos = rx.indexIn(str(str(line)), pos)
                        ...
                        if token in matches:
                            if type(matches[token]).__name__ != 'list':
                                tmp = matches[token]
                                matches[token] = [tmp]
                            matches[token].append(rx.indexIn(str(str(line)), pos))
                        else:
                            matches[token] = rx.indexIn(str(str(line)), pos)
